...
class TimelineLinearAnimation extends TimelineAnimation {
	...
	public function new(name,frame,totalDuration) {
	...
	override function clone(?a:Animation) {
		if( a == null )
			a = new TimelineLinearAnimation(name, frameCount, totalDuration);
		super.clone(a);
		return a;
	}
	...
	override function sync( decompose = false ) {
		if( frame == syncFrame && !decompose )
			return;
		#if debug_gltf
		trace("TLA.sync: frame="+frame+" syncFrame="+syncFrame);
		#end
		...
		for( o in getFrames() ) {
			...
			var frameSpan = o.frames[frame2].keyTime - o.frames[frame1].keyTime;
			var k2 = frameSpan==0 ? o.frames[frame1].keyTime : (frame - o.frames[frame1].keyTime) / frameSpan;
			var k1 = 1 - k2;
			syncFrame = frame;
			if( decompose ) isSync = false;

			#if debug_gltf
			trace(" - fLen:"+o.frames.length+" f1="+frame1+" f2="+frame2+" frameSpan="+frameSpan+" k2="+k2+" k1="+k1+" syncFrame="+syncFrame+" keyTime1="+o.frames[frame1].keyTime+" keyTime2="+o.frames[frame2].keyTime);
			#end
			...
			#if debug_gltf
			trace(" - translation: f1="+f1.tx+", "+f1.ty+", "+f1.tz+" f2="+f1.tx+", "+f1.ty+", "+f1.tz+" current="+m._41+", "+m._42+", "+m._43);
			#end
			
			if( o.hasRotation ) {
				#if debug_gltf
				trace(" - hasRotation:"+f1.qx+", "+f1.qy+", "+f1.qz+", "+f1.qw);
				#end
				// qlerp nearest
				...
				if( decompose ) {
					...
					if( o.hasScale ) {
						#if debug_gltf
						trace(" - hasScale:"+f1.sx+", "+f1.sy+", "+f1.sz);
						#end
