...
class TimelineLinearAnimation extends TimelineAnimation {
	...
	inline function getFrames() : Array<TimelineLinearObject> {
		return cast objects;
	}
	...
	public function mergeOrAddCurve( objName, frames:haxe.ds.Vector<TimelineLinearFrame>, hasPos, hasRot, hasScale, hasWeights ) {
		...
		for (o in objects) {
			...
			trace("CheckName="+tla.objectName+" match="+(tla.objectName == objName));
			// Check object names match
			if (tla.objectName == objName) {
				// If the anim already has pos/rot/scale/weights - add a new one
				if ((tla.hasPosition && hasPos) || (tla.hasRotation && hasRot) || (tla.hasScale && hasScale) || (tla.hasWeights && hasWeights)) {
					trace(" - Breaking from object loop - matched data set pos/rot/scale/weights");
					break;
				}
				...
				if (frames.length != tla.frames.length ) 
					...
					for (f in 0...frames.length) {
						var f1 = frames[f];
						var f2 = tla.frames[f];
						trace(" - Frameloop:"+f+" f.KT="+frames[f].keyTime+" o.f.KT="+tla.frames[f].keyTime);
						if (frames[f].keyTime != tla.frames[f].keyTime) {
							trace("   - Breaking from frame loop");
							frameMatch = false;
							break;
						}
					}
				...
				if (frameMatch) {
					for (f in 0...frames.length) {
						...
						trace("Frame-"+f+":"+objf.tx+" "+objf.ty+" "+objf.tz+" "+objf.qx+" "+objf.qy+" "+objf.qz+" "+objf.qw+" "+objf.sx+" "+objf.sy+" "+objf.sz);
						if (hasPos) {
						...
						trace("UpdatedFrame:"+objf.tx+" "+objf.ty+" "+objf.tz+" "+objf.qx+" "+objf.qy+" "+objf.qz+" "+objf.qw+" "+objf.sx+" "+objf.sy+" "+objf.sz);
					}
					merged = true;
				}

			}
			trace("end of object test - still in loop");
		}
	...
	override function sync( decompose = false ) {
		...
		for( o in getFrames() ) {
			...
			if( o.targetSkin != null ) {
				...
				// if( o.hasScale ) {
				// 	o.targetSkin.scaleX = ilerp(f1.sx, k1, f2.sx, k2, f1.t0x, f1.t1x);
				// 	o.targetSkin.scaleY = ilerp(f1.sy, k1, f2.sy, k2, f1.t0y, f1.t1y);
				// 	o.targetSkin.scaleZ = ilerp(f1.sz, k1, f2.sz, k2, f1.t0z, f1.t1z);
				// 	#if debug_gltf
				// 	trace(" - scale: f1="+f1.sx+", "+f1.sy+", "+f1.sz+" f2="+f2.sx+", "+f2.sy+", "+f2.sz+" current="+o.targetSkin.scaleX+", "+o.targetSkin.scaleY+", "+o.targetSkin.scaleY);
				// 	#end
				// } 
				// if (o.hasRotation) {
				// 	// qlerp nearest
				...
				// }
				// if (o.hasPosition) {
				// 	o.targetSkin.x = ilerp(f1.tx, k1, f2.tx, k2, f1.t0x, f1.t1x);
				// 	o.targetSkin.y = ilerp(f1.ty, k1, f2.ty, k2, f1.t0y, f1.t1y);
				// 	o.targetSkin.z = ilerp(f1.tz, k1, f2.tz, k2, f1.t0z, f1.t1z);
				// 	#if debug_gltf
				// 	trace(" - translation: f1="+f1.tx+", "+f1.ty+", "+f1.tz+" f2="+f2.tx+", "+f2.ty+", "+f2.tz+" current="+o.targetSkin.x+", "+o.targetSkin.y+", "+o.targetSkin.z);
				// 	#end
				// }

				// if (o.hasWeights) {
				// 	updateWeightData( o.targetSkin, f1, f2, k1, k2 );
				// }
