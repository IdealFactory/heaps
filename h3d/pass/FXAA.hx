...
private class TunedFXAAShader extends h3d.shader.ScreenShader {
    static var SRC = {
        @param var delta : Vec2;
        @param var texture : Sampler2D;
        @param var strength : Float;  // 0.0=off (raw sharp), 1.0=full FXAA (smoother but blurrier)
        ...
        function fragment() {
            ...
            // Key tweak: Scale dir by strength for tunable edge search (lower = sharper, less blur)
            dir *= strength;
...
class TunedFXAA extends ScreenFx<TunedFXAAShader> {
    public function new(strength:Float = 0.5) {  // Default half-strength for balance
        super(new TunedFXAAShader());
        this.strength = strength;
    }

    public var strength(default, set):Float = 0.5;
