...
class GltfTools {
	...
	public static function createBRDFTexture( s2d:h2d.Scene ) {
		...
		var brdfTexture = new h3d.mat.Texture(brdfBitmapData.width, brdfBitmapData.height, [h3d.mat.Data.TextureFlags.Target], h3d.mat.Texture.nativeFormat);
		...
		// @:privateAccess engine.width = brdfBitmapData.width;
		// @:privateAccess engine.height = brdfBitmapData.height;

		// @:privateAccess engine.needFlushTarget = true;

		// var driver:h3d.impl.GlDriver = cast engine.driver;
		// engine.clearF( new h3d.Vector() );
		...
		// @:privateAccess engine.width = oldW;
		// @:privateAccess engine.height = oldH;
